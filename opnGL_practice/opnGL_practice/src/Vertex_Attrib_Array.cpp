...
int main()
{
	...
	CallLog(glGenBuffers(1, &points_vao));
	CallLog(glBindBuffer(GL_ARRAY_BUFFER, points_vao));
	CallLog(glBufferData(GL_ARRAY_BUFFER, sizeof(positions), positions, GL_STATIC_DRAW));
	
	unsigned int colours_vao;
	CallLog(glGenBuffers(1, &colours_vao));
	CallLog(glBindBuffer(GL_ARRAY_BUFFER, colours_vao));
	CallLog(glBufferData(GL_ARRAY_BUFFER, sizeof(colours), colours, GL_STATIC_DRAW));
	...
	CallLog(glBindBuffer(GL_ARRAY_BUFFER, colours_vao));
	CallLog(glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(GL_FLOAT), 0));
	...
	const char* vertex_shader =
		...
		"layout (location = 1) in vec3 vertex_colours;\n"
	...
	glBindAttribLocation(program, 0, "vertex_colours");
